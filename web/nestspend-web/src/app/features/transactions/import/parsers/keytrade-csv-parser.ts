...
/**
 * Parser for Keytrade CSV bank exports.
 * Keytrade CSV format typically has columns like:
 * - Date
 * - Amount
 * - Counterparty
 * - Communication
 */
export class KeytradeCsvParser implements BankImportParser {
  ...
  canHandle(fileName: string, mimeType: string, firstLines: string[]): boolean {
    ...
    // Check if first lines contain Keytrade-specific headers
    const firstLinesLower = firstLines.join(' ').toLowerCase();
    return (
      firstLinesLower.includes('keytrade') ||
      KeytradeCsvParser.KNOWN_HEADERS.some((header) => firstLinesLower.includes(header))
    );
  }
  ...
  private detectColumns(headers: string[]): Record<string, number> {
    ...
    headers.forEach((header, index) => {
      if (header.includes('date') || header.includes('datum') || header.includes('valeur')) {
        map['date'] = index;
      }
      if (
      ...
      if (header.includes('iban') || header.includes('compte') || header.includes('rekening')) {
        map['iban'] = index;
      }
      if (header.includes('référence') || header.includes('reference') || header.includes('ref')) {
        map['reference'] = index;
      }
    });
